(() => {
      ...
      <div class="max-w-7xl mx-auto px-4">
        <div class="masthead">
          <div class="brand-lockup">
            <span class="brand-mark"><i class="fas fa-shield-alt" aria-hidden="true"></i></span>
            <div><div class="brand-name">SafeTrackerHub</div><div class="brand-network">Independent consumer trackers</div></div>
          </div>
        ...
        <div class="hero">
          <div>
            ...
            <h1>Find real chances. Avoid expensive mistakes.</h1>
            <p class="hero-copy">We review sweepstakes and rewards sites for transparency, fulfillment history, entry traps, realistic win opportunity, and marketing pressure.</p>
          </div>
    ...
    if (table && tableWrap) {
      ...
      rankings.insertAdjacentHTML("afterbegin", `
        <div class="rankings-head">
          ...
          <p class="updated-line">Reviewed ${updated} · Lower ScamFactor is better</p>
        </div>
        <div class="controls" aria-label="Ranking controls">
          <input id="tracker-search" class="control search-control" type="search" placeholder="Search sites, prizes, or red flags…" aria-label="Search tracker">
          ...
          <select id="sort-rankings" class="control" aria-label="Sort rankings">
            <option value="rank">Sort: Editorial rank</option><option value="score-low">Sort: Lowest ScamFactor</option><option value="score-high">Sort: Highest ScamFactor</option><option value="name">Sort: Site name</option>
          </select>
        </div>
      ...
      <footer id="disclosure" class="site-footer">
        <div class="max-w-7xl mx-auto px-4">
          <div class="footer-grid">
            <div><div class="footer-title">SafeTrackerHub</div><p class="footer-copy">Independent trackers that make risky, confusing online categories easier to compare. SweepSafe is our sweepstakes and rewards edition.</p></div>
            <div><div class="footer-title">Explore</div><div class="footer-links"><a href="#rankings">Rankings</a><a href="#editor-picks">Editor picks</a><a href="#methodology">Methodology</a></div></div>
  ...
  items.forEach((item) => {
    ...
    if (item.link) {
      ...
      favicon.src = new URL("/favicon.ico", item.link).href;
    ...
    const link = item.row.cells[7].querySelector("a");
    if (link) {
  ...
  function makeCard(item) {
    ...
    const favicon = item.link ? `<img class="site-favicon" src="${new URL("/favicon.ico", item.link).href}" alt="" width="20" height="20" loading="lazy" onerror="this.remove()">` : "";
    card.innerHTML = `
      <div class="mobile-card-head">
        <div><span class="mobile-rank">#${item.rank}</span><h3 class="mobile-site-name">${favicon}<span>${item.name}</span></h3><span class="category-badge">${item.category}</span>${badgeLabels[item.rank] ? `<span class="editor-badge">${badgeLabels[item.rank]}</span>` : ""}</div>
  ...
  function applyControls() {
    ...
    const ordered = [...items].sort((a, b) => {
      if (sort.value === "score-low") return a.score - b.score || a.rank - b.rank;
